...
if __name__ == "__main__":
    ...
    filename = 'pred_error_2th.txt'

    for a, b in unique_pairs.cpu().numpy():
        print(f"[{a}-{b}]")
        epsilons_a_b = epsilons[(epsilons[:, 1] == a) & (epsilons[:, 2] == b)]
        traj_data_a_b = traj_data[(epsilons[:, 1] == a) & (epsilons[:, 2] == b)]
        with torch.no_grad():
            init_state = traj_data_a_b[:, 0]
            for i in range(100):
                print(f"[{a}-{b}] step: {i}/100", end="\r")
                init_state = model.predict(init_state, epsilons_a_b)
