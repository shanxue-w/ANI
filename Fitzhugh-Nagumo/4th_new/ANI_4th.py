...
class FNO_ETDRK4(nn.Module):
    def __init__(self, modes1, modes2, width, N=128, Du=1e-2, Dv=1e-2, device='cuda:0'):
...
class FitzHughNagumo2D(torch.nn.Module):
    def __init__(self, N=128, device='cuda:1'):
        super().__init__()
        self.N = N
        self.device = device
        self._init_fft_grid()
...
class AllenCahn(nn.Module):
    ...
    def forward(self, x, eps):
        ...
        u = self.fno(u, eps)
        save_2 = u
        k1 = self.N0_SCHEME.single_step(u, dts=self.dt_tensor)
        save_3 = k1

        v = self.N0_SCHEME.single_step(x, dts=self.dt_tensor/2)
        save_4 = v
    ...
    def predict(self, x, eps):
        ...
        v = self.N0_SCHEME.single_step(x, dts=self.dt_tensor/2)
        v = self.fno(v, eps, dt=1e-2/2)
        v = self.N0_SCHEME.single_step(v, dts=self.dt_tensor)
        v = self.fno(v, eps, dt=1e-2/2)
        k2 = self.N0_SCHEME.single_step(v, dts=self.dt_tensor/2)
        return -1.0/3.0 * k1 + 4.0/3.0 * k2
