...
class AllenCahn(nn.Module):
    ...
    def forward(self, x, eps):
        x1 = self.N0_SCHEME.single_step(x, dts=self.dt_tensor)
        x1 = self.N0_SCHEME.single_step(x1, dts=self.dt_tensor)
        x2 = self.fno(x, eps)
        return x1 + x2
...
if __name__ == "__main__":
    ...
    for epoch in range(epochs):
        ...
        if val_loss < best_val_loss:
            ...
            torch.save(model.state_dict(), os.path.join('models', "best_model_new_RK4.pth"))
