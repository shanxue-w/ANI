...
def evaluate_on_trajectory(model, init_states, epsilons, true_trajs, dt=0.01):
    ...
    for t in range(T):
        ...
        next_states = model(curr_states, epsilons)
...
if __name__ == "__main__":
    ...
    traj_data = np.load('../dataset/data/fhn_test_trajectory_6_new.npy')
    epsilons  = np.load('../dataset/data/fhn_test_eps_6_new.npy')
    ...
    # 写入文件，每行写一个时间点的误差
    with open("traj_error_6.txt", "w") as f:
