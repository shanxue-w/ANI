...
MODEL_FILE = f"Baseline_Best_{BASIN_ID}.pth"
...
class Baseline_Hydro(nn.Module):
    ...
    def _strang_step(self, q, x_raw_t, x_norm_t, history_norm, dt):
        ...
        # q_half = self.A(q, P_raw, T_raw, dt)
        q_half_norm = (q - self.y_mean) / (self.y_std + 1e-12)
        drift = self.NN(history_norm, q_half_norm, x_norm_t)  # [B,1]
        q_corr = q + drift * dt * self.y_std

        return F.softplus(q_corr), drift
...
if __name__ == "__main__":
    ...
    model = Baseline_Hydro(full_data['ode_prior'], full_data['stats'], input_dim, HIDDEN_DIM).to(device)
