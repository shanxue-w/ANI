...
class ANI_4th_Hydro(nn.Module):
    ...
    def forward(self, history_norm, x_future_raw, x_future_norm, q_start, pred_steps, dt=1.0):
        ...
        for t in range(pred_steps):
            ...
            q_c, d_c = self._strang_step(curr_q, x_raw_t, x_norm_t, curr_history, DT)

            q_1, d_1 = self._strang_step(curr_q, x_raw_t, x_norm_t, curr_history, DT/2)
            q_2, d_2 = self._strang_step(q_1, x_raw_t, x_norm_t, curr_history, DT/2)
            
            curr_q = -1/3 * q_c + 4/3 * q_2
            
            preds.append(curr_q)
            drifts.extend([d_c, d_1, d_2])
...
if __name__ == "__main__":
    ...
    model = ANI_4th_Hydro(full_data['ode_prior'], full_data['stats'], input_dim, HIDDEN_DIM).to(device)
