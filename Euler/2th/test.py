...
from ANI_Euler_2th import Euler, A  # 假设你的模型类叫 Euler 并保存在 model.py 中
import os
from pit import *
from utils import *
import time
...
def fun_Pri(x, k=3):
    ...
    # rho_vals = np.random.uniform(0.1, 1.0, k + 1)
    # u_vals   = np.random.uniform(-1.0, 1.0, k + 1)
    # p_vals   = np.random.uniform(0.1, 1.0, k + 1)
...
def compute_dt_phys(phys, dx, CFL=0.5, gamma=1.4):
    """
    计算时间步长
    """
    rho, u, p = phys[:, :, 0], phys[:, :, 1], phys[:, :, 2]
    c = torch.sqrt(gamma * p / rho)
    dt = CFL * dx / torch.max(torch.abs(u) + c)
    return dt.item()

def compute_dt_cons(cons, dx, CFL=0.5, gamma=1.4):
    """
    计算时间步长
    """
    rho, m, E = cons[:, :, 0], cons[:, :, 1], cons[:, :, 2]
    u = m / rho
    c = torch.sqrt(gamma * (E - 0.5 * rho * u**2) / rho)
    dt = CFL * dx / torch.max(torch.abs(u) + c)
    return dt.item()
...
def main():
    ...
    model.load_state_dict(torch.load("best_model_test_new.pth", map_location=device))
    model.eval()
    ...
    while t < t_end:
        dt_value = compute_dt_cons(u, dx, CFL=0.4)      # float
        if t + dt_value > t_end:
            dt_value = t_end - t
    ...
    # np.save("error_rho_sod_2th.npy", error_rho)
    # np.save("error_u_sod_2th.npy", error_u)
    # np.save("error_p_sod_2th.npy", error_p)
    ...
    # plt.legend()
    # plt.tight_layout()
    # plt.savefig("riemann_trajectory_2th_sod.png")

    # np.save("rho_sod_2th.npy", rho)
    # np.save("rho_sod.npy", rho_out)
    # np.save("u_sod_2th.npy", u)
    # np.save("u_sod.npy", u_out1)
    # np.save("p_sod_2th.npy", p)
    # np.save("p_sod.npy", p_out)
    ...
    #     plt.legend()
    # plt.tight_layout()
    # plt.savefig("riemann_trajectory_error_2th_2.png")  
...
def data():
    ...
    for i in range(100):
        ...
        u_np = torch.tensor(u_out, dtype=torch.float64)  # [N, 3]
        pri = Con2Pri(u_np)        # [N, 3]
        # phy_rho, phy_u, phy_p = pri[:, 0], pri[:, 1], pri[:, 2]
        data_list.append(pri)
    data_array = np.array(data_list)
    print(data_array.shape)
    np.save("sod_weno.npy", data_array)
