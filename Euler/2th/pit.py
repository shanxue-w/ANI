...
class pit(nn.Module):
    def __init__(self,
        ...
        self.film_mlps = nn.ModuleList([
            FiLM_MLP(1, self.hid_dim, self.hid_dim) for _ in range(self.n_blocks)
        ])
    ...
    def processor(self, func_ltt, mesh_ltt, dts):
        for a, w, film in zip(self.conv, self.mlp, self.film_mlps):
            ...
            gamma, beta = film(dts.unsqueeze(-1))      # [B, 1, C]
            func_ltt = gamma * func_ltt + beta
