...
Location, rho_new1, u_new1, p_new1, e_new1 = np.loadtxt(
    'Exact/res1.csv', 
    delimiter=',', 
    skiprows=1, 
    unpack=True
...
error_rho_4th = np.abs(rho_4th - rho_new1)
error_u_4th   = np.abs(u_4th - u_new1)
error_p_4th   = np.abs(p_4th - p_new1)

error_rho_2th = np.abs(rho_2th - rho_new1)
error_u_2th   = np.abs(u_2th - u_new1)
error_p_2th   = np.abs(p_2th - p_new1)

error_rho_tvd = np.abs(rho_tvd - rho_new1)
error_u_tvd   = np.abs(u_tvd - u_new1)
error_p_tvd   = np.abs(p_tvd - p_new1)
...
plt.tight_layout()
plt.savefig("error_comparison_new1.pdf", format='pdf', bbox_inches='tight', dpi=300)
# plt.show()
...
plt.tight_layout()
# plt.show()
plt.savefig("solution_comparison_new1.pdf", format='pdf', bbox_inches='tight', dpi=300)

fig = plt.figure(figsize=(2, 2))
plt.plot(rho_new1, color="#0072B2")
plt.axis("off")
plt.gca().set_frame_on(False)
plt.tight_layout()
plt.savefig("data_1_new1.pdf", format='pdf', bbox_inches='tight', dpi=300, transparent=True)
plt.close()

plt.figure(figsize=(2, 2))
plt.plot(u_new1, color="#0072B2")
plt.axis("off")
plt.gca().set_frame_on(False)
plt.tight_layout()
plt.savefig("data_2_new1.pdf", format='pdf', bbox_inches='tight', dpi=300, transparent=True)
plt.close()

plt.figure(figsize=(2, 2))
plt.plot(p_new1, color="#0072B2")
plt.axis("off")
plt.gca().set_frame_on(False)
plt.tight_layout()
plt.savefig("data_3_new1.pdf", format='pdf', bbox_inches='tight', dpi=300, transparent=True)
plt.close()
