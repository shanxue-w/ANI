...
# class A(N0):
#     def __init__(self, mu=0.0, sigma=1.0, gamma=1.4, dx=1/128, dt=0.025, Ng=1):
#         super().__init__()
#         self.mu = mu
#         self.sigma = sigma
#         self.gamma = gamma
#         self.dx = dx
#         self.dt = dt
#         self.Ng = Ng  # Lax-Friedrichs 只需 1 个 ghost cell

#     def _primitive_and_flux(self, u):
#         """同时计算 v, p, flux 三者，避免重复开销"""
#         rho = torch.clamp(u[..., 0], min=1e-6)
#         mom = u[..., 1]
#         E   = u[..., 2]
...
#         f2 = mom * v + p
#         f3 = v * (E + p)
#         flux = torch.stack([f1, f2, f3], dim=-1)

#         return v, p, flux

#     def compute_alpha(self, v, p, rho):
#         c = torch.sqrt(self.gamma * p / rho + 1e-8)
#         return torch.abs(v) + c  # 波速

#     def lax_friedrichs_flux(self, u_L, u_R):
#         # 左右状态同时计算通量和波速估计
#         vL, pL, f_L = self._primitive_and_flux(u_L)
#         vR, pR, f_R = self._primitive_and_flux(u_R)

#         rhoL = torch.clamp(u_L[..., 0], min=1e-6)
#         rhoR = torch.clamp(u_R[..., 0], min=1e-6)
#         alpha_L = self.compute_alpha(vL, pL, rhoL)
#         alpha_R = self.compute_alpha(vR, pR, rhoR)
...
#         fluxes = self.lax_friedrichs_flux(u_L, u_R)  # [B, Nx+2Ng-1, 3]
#         rhs = -(fluxes[:, 1:, :] - fluxes[:, :-1, :]) / self.dx  # [B, Nx, 3]
#         return rhs

#     def single_step(self, u: torch.Tensor, dts: torch.Tensor) -> torch.Tensor:
#         Ng = self.Ng
#         # 只做一次 pad（等价于原来 pad + update_boundaries）
#         u_padded = F.pad(u, pad=(0, 0, Ng, Ng), mode="replicate")

#         if dts.dim() == 1:
#             dts_expanded = dts[:, None, None]  # [B, 1, 1]
...
#     # def single_step(self, u: torch.Tensor, dts: torch.Tensor) -> torch.Tensor:
#     #     """
#     #     TVD MUSCL 空间离散 + SSPRK3 时间积分
#     #     u: [batch, Nx, 3] -> 自动 pad -> [batch, Nx+2*Ng, 3]
#     #     返回: [batch, Nx, 3] (只返回物理域)
#     #     """
#     #     Ng = self.Ng
#     #     # 对输入 u 进行 pad，包含 ghost cells
#     #     u_padded = F.pad(u, pad=(0, 0, Ng, Ng), mode="replicate")
#     #     self.update_boundaries(u_padded) # 确保初始边界条件设置

#     #     # 安全地扩展 dts 以进行广播
#     #     if dts.dim() == 1:
#     #         dts_expanded = dts.unsqueeze(-1).unsqueeze(-1) # -> [B, 1, 1]
#     #     elif dts.dim() == 3 and dts.shape[1] == 1 and dts.shape[2] == 1:
#     #         dts_expanded = dts # 已经是正确的 [B, 1, 1] 形状
#     #     else:
#     #         raise ValueError(f"dts 的形状不符合预期: {dts.shape}. 预期 [B] 或 [B, 1, 1].")

#     #     # SSPRK3 阶段
#     #     # 阶段 1
#     #     rhs1 = self.compute_rhs(u_padded)
#     #     u1_physical = u + dts_expanded * rhs1
#     #     u1_padded = F.pad(u1_physical, pad=(0, 0, Ng, Ng), mode="replicate")
#     #     self.update_boundaries(u1_padded) # 更新 u1_physical 的边界

#     #     # 阶段 2
#     #     rhs2 = self.compute_rhs(u1_padded)
#     #     u2_physical = 0.75 * u + 0.25 * u1_physical + 0.25 * dts_expanded * rhs2
#     #     u2_padded = F.pad(u2_physical, pad=(0, 0, Ng, Ng), mode="replicate")
#     #     self.update_boundaries(u2_padded) # 更新 u2_physical 的边界

#     #     # 阶段 3
#     #     rhs3 = self.compute_rhs(u2_padded)
#     #     u_new_physical = (1/3) * u + (2/3) * u2_physical + (2/3) * dts_expanded * rhs3

#     #     return u_new_physical # 只返回物理域
#     def single_step(self, u: torch.Tensor, dts: torch.Tensor) -> torch.Tensor:
#         Ng = self.Ng
#         u_padded = F.pad(u, pad=(0, 0, Ng, Ng), mode="replicate")
...
class Euler(nn.Module):
    ...
    def forward(self, x):
        ...
        k2 = self.block_tau(self.block_tau(u, dts/2), dts/2)
...
if __name__ == "__main__":
    ...
    for epoch in range(epochs):
        ...
        if val_loss < best_val_loss:
            ...
            torch.save(model.state_dict(), "best_model_best.pth")
