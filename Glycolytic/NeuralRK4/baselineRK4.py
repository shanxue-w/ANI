...
class RK4Baseline(nn.Module):
    def __init__(self, input_dim=7, output_dim=7, hidden_dim=128, hidden_layers=4):
...
if __name__ == "__main__":
    ...
    # # 初始化模型
    model = RK4Baseline(input_dim=7, output_dim=7, hidden_layers=6, hidden_dim=32).to(device)
    ...
    with torch.no_grad():
        ...
        for traj in test_trajectories:
            ...
            if j == 0:
                np.save("u_baseline.npy", u_lists)
                # exit(-1)
