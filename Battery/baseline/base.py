...
if __name__ == "__main__":
    ...
    prior = A(R_val, C_val, w_vals).to(device)
    ...
    with torch.no_grad():
        ...
        prior_pred = prior(x_sample).squeeze(-1).cpu().numpy()
        ...
        plt.plot(np.abs(ani2_pred - y_true), 'g--', label='Baseline (Prior+Res)', alpha=0.8)
        plt.legend()
        plt.title("Test Set Prediction: Prior vs Baseline")
        plt.xlabel("Sample Index")
        plt.ylabel("Voltage (V)")
        plt.savefig("baseline_result.png")
        print("Result plot saved to baseline_result.png")