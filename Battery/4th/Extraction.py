...
from ANI4 import A, load_data, DATA_PATH
...
class ANI2(nn.Module):
    ...
    def forward(self, x):
        ...
        u1, dvdt1, cond1 = self.strang_step(x)
        
        x2 = x.clone()
        x2[:, 4:5] = dt_half
        u2_half, dvdt2_a, cond2_a = self.strang_step(x2)

        x3 = x.clone()
        x3[:, 0:1] = u2_half
        x3[:, 4:5] = dt_half
        u2, dvdt2_b, cond2_b = self.strang_step(x3)

        all_dvdt = torch.cat([dvdt1, dvdt2_a, dvdt2_b], dim=0)
        all_cond = torch.cat([cond1, cond2_a, cond2_b], dim=0)

        return 4/3*u2 - 1/3*u1, all_dvdt, all_cond
...
# Model checkpoint
MODEL_CKPT_PATH = "best_ani4_model.pth" 
