...
class ANI4(nn.Module):
    ...
    def forward(self, x):
        ...
        u1 = self.strang_step(x)
        
        x2 = x.clone()
        x2[:, 4:5] = dt_half
        u2_half = self.strang_step(x2)

        x3 = x.clone()
        x3[:, 0:1] = u2_half
        x3[:, 4:5] = dt_half
        u2 = self.strang_step(x3)

        return 4/3*u2 - 1/3*u1
...
if __name__ == "__main__":
    ...
    with torch.no_grad():
        ...
        plt.plot(np.abs(ani2_pred - y_true), 'g--', label='ANI4 (Prior+Res)', alpha=0.8)
        plt.legend()
        plt.title("Test Set Prediction: Prior vs ANI4")
        plt.xlabel("Sample Index")
        plt.ylabel("Voltage (V)")
        plt.savefig("ani4_result.png")
        print("Result plot saved to ani4_result.png")
