...
class A(N0):
    def __init__(self, Nx=256, Ny=512, Lx=1.0, Ly=2.0, Re=1e4, device='cuda:1'):
        super().__init__()
...
class NavierStokes(nn.Module):
    ...
    def net(self, omega, x, dt):
        ...
        delta = self.fno(x, dt)
        delta_x = dt * delta[:, 0:1]
        delta_y = dt * delta[:, 1:2]
        residual = dt * delta[:, 2:3]
        ...
        # out = torch.fft.ifft2(omega_new, norm="ortho").real.squeeze(1) + residual.squeeze(1)
        # out_mean = torch.mean(out, dim=(1,2))
        # out = out + omega_mean[:, None, None] - out_mean[:, None, None]
    ...
    def forward(self, x):
        ...
        omega_features = torch.stack([omega, u, v], dim=1)  # [B, H, W, 2]
