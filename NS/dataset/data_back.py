...
class A(N0):
    def __init__(self, Nx=1024, Ny=1024, Lx=1.0, Ly=1.0, Re=1e4, dt_step=1e-4, device='cuda:1', factor=1):
        super().__init__()
    ...
    def etdrk4_step(self, omega: torch.Tensor, dt: float) -> torch.Tensor:
        ...
        if math.isclose(dt, self.dt_step):
            ...
            E, E2, phi1, _, _, A_coeffs, B_coeffs, C_coeffs = self._cached_dt_coeffs
        else:
            E, E2, phi1, _, _, A_coeffs, B_coeffs, C_coeffs = self._compute_etdrk4_coefficients(dt)
...
#                                         max_radius=0.10, max_strength=20.0, device='cuda:1'):
#     x = torch.linspace(0, Lx, Nx+1, device=device)[:-1]
#     y = torch.linspace(0, Ly, Ny+1, device=device)[:-1]
#     X, Y = torch.meshgrid(x, y, indexing='ij')  # [Nx, Ny]
...
def generate_initial_velocity(batch_size, Nx=1024, Ny=1024, Lx=1.0, Ly=1.0, device='cuda:1'):
    """
