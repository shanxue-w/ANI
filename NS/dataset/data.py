...
# def generate_kolmogorov_flow_vorticity(batch_size, Nx=256, Ny=256, Lx=1.0, Ly=1.0,
#                                      k_min=2, k_max=10,
#                                      n_small_vortices=100, small_radius=(0.01,0.03),
#                                      max_strength=5.0, noise_level=0.2,
#                                      device='cuda:0', seed=None):
#     """
#     生成全域密集、多尺度随机涡量场：
#     - 波数随机叠加覆盖整个域
#     - 小尺度 Gaussian 涡增加局部极值
#     - 小噪声增加随机性
#     """
#     if seed is not None:
#         torch.manual_seed(seed)
...
#             kx = torch.randint(k_min, k_max+1, (1,)).item()
#             ky = torch.randint(k_min, k_max+1, (1,)).item()
#             phase = 2*math.pi*torch.rand(1, device=device)
#             strength = (2*torch.rand(1, device=device)-1) * max_strength / 2
#             omega[b] += strength * torch.sin(2*math.pi*(kx*X/Lx + ky*Y/Ly) + phase)

#         # # --- 小尺度 Gaussian 涡 ---
#         # for _ in range(n_small_vortices):
#         #     x0 = torch.rand(1, device=device) * Lx
#         #     y0 = torch.rand(1, device=device) * Ly
#         #     radius = small_radius[0] + (small_radius[1] - small_radius[0]) * torch.rand(1, device=device)
#         #     strength = (2*torch.rand(1, device=device)-1) * max_strength
#         #     r2 = (X - x0)**2 + (Y - y0)**2
...
def generate_kolmogorov_flow_vorticity(batch_size, Nx=1024, Ny=1024, Lx=1.0, Ly=1.0,
                                        n_large_vortices=3, n_small_vortices_range=(5, 20),
                                        large_radius_range=(0.08, 0.12), small_radius_range=(0.01, 0.05),
                                        large_strength_range=(10.0, 20.0), small_strength_range=(1.0, 5.0),
                                        device='cuda'):
    x = torch.linspace(0, Lx, Nx+1, device=device)[:-1]
    y = torch.linspace(0, Ly, Ny+1, device=device)[:-1]
    X, Y = torch.meshgrid(x, y, indexing='ij')  # [Nx, Ny]
    ...
    for b in range(batch_size):
        # --- 大涡 ---
        for _ in range(n_large_vortices):
            x0 = torch.rand(1, device=device) * Lx
            y0 = torch.rand(1, device=device) * Ly
            radius = torch.rand(1, device=device) * (large_radius_range[1] - large_radius_range[0]) + large_radius_range[0]
            strength = (torch.rand(1, device=device) - 0.5) * 2 * (large_strength_range[0] + large_strength_range[1]) / 2
            r2 = (X - x0)**2 + (Y - y0)**2
            blob = strength * torch.exp(-r2 / (2 * radius**2))
            omega[b] += blob

        # --- 小涡 ---
        n_small = torch.randint(n_small_vortices_range[0], n_small_vortices_range[1]+1, (1,)).item()
        for _ in range(n_small):
            x0 = torch.rand(1, device=device) * Lx
            y0 = torch.rand(1, device=device) * Ly
            radius = torch.rand(1, device=device) * (small_radius_range[1] - small_radius_range[0]) + small_radius_range[0]
            strength = (torch.rand(1, device=device) - 0.5) * 2 * small_strength_range[1]
            r2 = (X - x0)**2 + (Y - y0)**2
            blob = strength * torch.exp(-r2 / (2 * radius**2))
            omega[b] += blob
...
def generate_and_save_dataset(model, batch_size, batch, steps, dt, device, save_dir, split_name):
    ...
    while batch_idx < batch:
        ...
        for step in range(steps):
            ...
            trajectory_list.append(u.cpu().numpy()[:,:,::4,::4])  # [B, 2, Nx, Ny]
    ...
    # 6. 保存
    torch.save(input_tensor, os.path.join(save_dir, f"{split_name}_input.pt"))
    torch.save(output_tensor, os.path.join(save_dir, f"{split_name}_output.pt"))
...
def generate_test_dataset(model, batch_size, batch, steps, dt, device, save_path):
    ...
    while batch_idx < batch:
        ...
        for step in range(steps):
            ...
            trajectories.append(u.cpu()[:, :, ::4, ::4])  # 下采样
            u, status = model.single_step(u, dts=dt)
            if status == False:
                break
...
if __name__ == '__main__':
    ...
    generate_test_dataset(model, batch_size=1, batch=4, steps=400, dt=dt, device=device, save_path='test_trajectory.pt')
