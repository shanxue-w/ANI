...
from baseline import NavierStokes, A
import numpy as np
import matplotlib.pyplot as plt
...
def relative_l2_error(pred, true):
    ...
    # return torch.sqrt( mse_loss(pred, true) / mse_loss(true, torch.zeros_like(true)) )
...
def evaluate_k_step_rollout(model, traj_data, k=5):
    ...
    for t_start in tqdm(range(T - k), desc=f"Rolling out {k} steps"):
        ...
        for step in range(1, k + 1):
            ...
            curr_states = model.predict(curr_states)
...
if __name__ == "__main__":
    ...
    model.load_state_dict(torch.load(f"models/best_model_fno_4000_loadbefore.pth", map_location=device))
