...
class A(N0):
    def __init__(self, Nx=256, Ny=512, Lx=1.0, Ly=2.0, Re=1e4, device='cuda'):
        super().__init__()
...
class NavierStokes(nn.Module):
    def __init__(self, N0_SCHEME: N0, modes1=16, modes2=16, width=64, dt=0.1, device=device):
        ...
        self.fno = FNO2d(modes1=modes1, modes2=modes2, width=width, film_input=1, in_channels=3, out_channels=3).to(device)
    ...
    def forward(self, x):
        ...
        # Separate the displacement into x and y components
        delta_x = self.dt_tensor * delta[:, 0:1]
        delta_y = self.dt_tensor * delta[:, 1:2]
        residual = self.dt_tensor * delta[:, 2:3]

        # Apply the displacement using a spectral phase shift (FFT interpolation)
        # omega_fft = torch.fft.fft2(omega, norm="ortho").unsqueeze(1)
        
        # # Calculate the phase shift for each displacement component
        # phase = self.Kx * delta_x + self.Ky * delta_y
        # phase_shift = torch.exp(-1j * phase)
        
        # # Apply the phase shift to the spectral vorticity
        # omega_new = omega_fft * phase_shift
        
        # # # Convert the shifted vorticity back to the spatial domain
        # omega_1 = torch.fft.ifft2(omega_new, norm="ortho").real.squeeze(1) + residual.squeeze(1)

        # omega [B, H, W] -> mean [B]
        # mean_omega = torch.mean(omega, dim=(1, 2))
        # mean_omega_1 = torch.mean(omega_1, dim=(1, 2))

        # omega = omega_1 - mean_omega_1[:, None, None] + mean_omega[:, None, None]

        # omega = omega + residual.squeeze(1)

        x_origin = (self.x_phys - delta_x.squeeze(1)) % self.Lx
        y_origin = (self.y_phys - delta_y.squeeze(1)) % self.Ly
        x_norm   = 2.0 * (x_origin / self.Lx) - 1.0
        y_norm   = 2.0 * (y_origin / self.Ly) - 1.0
        grid     = torch.stack([x_norm, y_norm], dim=-1)  # [B, H, W, 2]

        # omega [B, 256, 256] -> omega_extend [B, 257, 257] 周期延拓过去就好
        omega_extend = torch.nn.functional.pad(omega.unsqueeze(1), (0, 1, 0, 1), mode='circular')
        omega_new = F.grid_sample(
            omega_extend, grid, mode='bilinear', padding_mode='zeros', align_corners=True
        ).squeeze(1) + residual.squeeze(1)
        # mean_new = omega_new.mean(dim=(-2, -1), keepdim=True)
        # omega_new = omega_new + residual.squeeze(1)
        mean_new = omega_new.mean(dim=(-2, -1), keepdim=True)
