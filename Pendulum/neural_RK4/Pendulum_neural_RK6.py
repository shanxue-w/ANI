...
#     def __init__(self, func, dt=5e-2, solver="rk4"):
#         super().__init__()
#         self.func = func
#         self.dt = dt
#         self.solver = solver
...
class NeuralRK4(nn.Module):
    def __init__(self):
        super().__init__()
        self.mlp = SimpleMLP()

    def forward(self, u, dts):
        # k1 = self.mlp(u)
        # k2 = self.mlp(u + 0.5 * dts * k1)
        # k3 = self.mlp(u + 0.5 * dts * k2)
        # k4 = self.mlp(u + dts * k3)
        # return u + (1/6) * dts * (k1 + 2*k2 + 2*k3 + k4)
        k1 = dts * self.mlp(u)
        k2 = dts * self.mlp(u + 1/3 * k1)
        k3 = dts * self.mlp(u + 2/3 * k2)
        k4 = dts * self.mlp(u + 1/12 * k1 + 1/3 * k2 - 1/12 * k3)
        k5 = dts * self.mlp(u - 1/16 * k1 + 9/8 * k2 - 3/16 * k3 - 3/8 * k4)
        k6 = dts * self.mlp(u + 9/8 * k2 - 3/8 * k3 - 3/4 * k4 + 1/2 * k5)
        k7 = dts * self.mlp(u + 9/44 * k1 - 9/11 * k2 + 63/44 * k3 + 18/11 * k4 - 16/11 * k6)
...
def train(model, train_loader, val_loader, criterion, optimizer, scheduler, epochs=3000):
    best_val = float("inf")
    dts      = torch.full((16,1), 1e-1, dtype=torch.float64, device=device)
...
def rollout(model, u0, steps=1000):
    ...
    dt = torch.full((1,1), 1e-1, dtype=torch.float64, device=device)
...
def plot_trajectory(pred, true, i=0):
    plt.figure()
    plt.plot(pred[:, 0], pred[:, 1], label="NeuralRK4", color="blue")
...
def main():
    ...
    with torch.no_grad():
        B, T, D = test_trajs.shape
        dt = 1e-1
        # 初始化 u
        u = test_trajs[:, 0, :]       # [B, D]
        # pred_trajs = [u]                     # list 保存所有时间步预测
        u = torch.tensor(u, device=device, dtype=torch.float64)
        dts = torch.full((B,1), dt, dtype=torch.float64, device=device)
        for t in range(1, T):
            # 拼接 dt
