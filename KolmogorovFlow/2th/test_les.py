...
from ANI2 import NavierStokes, A
import numpy as np
...
def evaluate_on_trajectory(model, init_states, true_trajs, dt=5e-3):
    ...
    # u_99, v_99 = A1._vorticity_to_velocity_spectral(pred_trajs[0, 99], A1.Kx_fine, A1.Ky_fine, A1.denom_safe_fine)

    # # u_149 = pred_trajs[0, 149, 0, :, :]
    # # v_149 = pred_trajs[0, 149, 1, :, :]
    # u_149, v_149 = A1._vorticity_to_velocity_spectral(pred_trajs[0, 149], A1.Kx_fine, A1.Ky_fine, A1.denom_safe_fine)

    # # u_199 = pred_trajs[0, 199, 0, :, :]
    # # v_199 = pred_trajs[0, 199, 1, :, :]
    # u_199, v_199 = A1._vorticity_to_velocity_spectral(pred_trajs[0, 199], A1.Kx_fine, A1.Ky_fine, A1.denom_safe_fine)

    # # u_249 = pred_trajs[0, 249, 0, :, :]
    # # v_249 = pred_trajs[0, 249, 1, :, :]
    # u_249, v_249 = A1._vorticity_to_velocity_spectral(pred_trajs[0, 249], A1.Kx_fine, A1.Ky_fine, A1.denom_safe_fine)

    # # u_299 = pred_trajs[0, 299, 0, :, :]
    # # v_299 = pred_trajs[0, 299, 1, :, :]
    # u_299, v_299 = A1._vorticity_to_velocity_spectral(pred_trajs[0, 299], A1.Kx_fine, A1.Ky_fine, A1.denom_safe_fine)

    # # u_349 = pred_trajs[0, 349, 0, :, :]
    # # v_349 = pred_trajs[0, 349, 1, :, :]
    # u_349, v_349 = A1._vorticity_to_velocity_spectral(pred_trajs[0, 349], A1.Kx_fine, A1.Ky_fine, A1.denom_safe_fine)
    ...
    # random choose one, use constant seed, 1-20
    ...
    for t, suffix in zip(time_steps, file_suffixes):
        ...
        np.save(f'u_{suffix}_les.npy', u_pred.cpu().numpy())
        np.save(f'v_{suffix}_les.npy', v_pred.cpu().numpy())
    ...
    # np.save('u0_les.npy', u0.cpu().numpy())
    # np.save('v0_les.npy', v0.cpu().numpy())
    # np.save('u99_les.npy', u_99.cpu().numpy())
    # np.save('v99_les.npy', v_99.cpu().numpy())
...
if __name__ == "__main__":
    ...
    traj_data = torch.load("../dataset/test_omega_traj_4x800x128x128.pt").to(device).to(torch.float64).unsqueeze(2)
    ...
    # 写入文件，每行写一个时间点的误差
    # with open("traj_error_les.txt", "w") as f:
