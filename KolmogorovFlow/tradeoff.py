...
# 定义模型配置
model_configs = [
    ('baseline', 'Baseline (FNO)', '#E69F00', 'o'),  # 橙色
...
results = {
    'baseline': {'sizes': [], 'times': [], 'errors': []},
    '2th': {'sizes': [], 'times': [], 'errors': []},
    # '4th': {'sizes': [], 'times': [], 'errors': []}
}

for folder, label, _, _ in model_configs:
    current_sizes = sizes_baseline_extra if folder == 'baseline' else sizes_common
    for size in current_sizes:
        ...
        error_path = os.path.join(root_dir, folder, f"k_1_error_{size}.txt")
        error_val = parse_error_from_traj(error_path)
...
# 分别调用生成
create_standard_plot('times', 'Training Cost Analysis', 'Training Time (Seconds)', 'scaling_time')
create_standard_plot('errors', 'Data Efficiency Analysis', r'Relative $L^2$ Error', 'scaling_error')