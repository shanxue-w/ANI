...
from ANI_2th import NavierStokes, A
import numpy as np
...
def evaluate_on_trajectory(model, init_states, true_trajs, dt=0.1):
    ...
    # np.save('u_99_les.npy', u_99.cpu().numpy())
    # np.save('v_99_les.npy', v_99.cpu().numpy())
    # np.save('u_149_les.npy', u_149.cpu().numpy())
    # np.save('v_149_les.npy', v_149.cpu().numpy())
    # np.save('u_199_les.npy', u_199.cpu().numpy())
    # np.save('v_199_les.npy', v_199.cpu().numpy())
    # np.save('u_249_les.npy', u_249.cpu().numpy())
    # np.save('v_249_les.npy', v_249.cpu().numpy())
    # np.save('u_299_les.npy', u_299.cpu().numpy())
    # np.save('v_299_les.npy', v_299.cpu().numpy())
    # np.save('u_349_les.npy', u_349.cpu().numpy())
    # np.save('v_349_les.npy', v_349.cpu().numpy())
...
if __name__ == "__main__":
    ...
    # 加载 trajectory 数据
    traj_data = torch.load("../dataset/test_trajectory_new.pt").to(device).to(torch.float64)   # [N, T, 128, 128]
    ...
    # # 写入文件，每行写一个时间点的误差
    # with open("traj_error_1e4.txt", "w") as f:
