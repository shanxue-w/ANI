...
class NavierStokes(nn.Module):
    def __init__(self, N0_SCHEME: N0, modes1=16, modes2=16, width=64, dt=5e-3, device=device):
    ...
    def forward(self, x):
        ...
        omega1 = self.N0_SCHEME.single_step(omega, dts=self.dt)
        ...
        omega_features = torch.stack([omega, u, v, x_grid, y_grid], dim=1)  # [B, H, W, 2]
        omega = self.net(omega, omega_features, self.dt_tensor/2).unsqueeze(1)
        omega2 = self.N0_SCHEME.single_step(omega, dts=self.dt_half)
