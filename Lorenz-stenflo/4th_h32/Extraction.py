...
model = LorenzStenflo(N0_SCHEME=A(mu, sigma), input_dim=5, output_dim=4, hidden_layers=4, hidden_dim=32).to(device)
...
try:
    # Older PySINDy versions accept feature_names in constructor.
    model = ps.SINDy(
    ...
    # Newer versions remove this constructor argument.
    model = ps.SINDy(
        feature_library=feature_library,
        optimizer=optimizer,
    )
...
try:
    # Older PySINDy versions
    model.fit(train_trajectories, t=dt_val, multiple_trajectories=True)
except TypeError:
    # Newer versions infer multi-trajectory from list input
    model.fit(train_trajectories, t=dt_val)
...
try:
    model.print(feature_names=["x", "y", "z", "w"])
except TypeError:
    model.print()
