...
class A(N0):
    ...
    def F(self, x: torch.Tensor):
        ...
        dx = torch.stack([dx1, dx2, dx3, dx4], dim=-1)  # 一次性合并

        return dx / self.sigma  # 最后再标准化
...
class CompoundPendulum(ANIBASE):
    def __init__(self, N0_SCHEME: N0, input_dim: int, output_dim: int, hidden_layers: int = 4, hidden_dim: int = 64):
        super(CompoundPendulum, self).__init__(N0_SCHEME, input_dim, output_dim, hidden_layers, hidden_dim)
    ...
    def forward(self, x):
        ...
        # 构造输入 [u, dts], [u, dts/3], [u2_step1, 2dts/3]
        u_repeat = u.repeat(3, 1)
        dts_all = torch.cat([dts, dts/3.0, 2.0*dts/3.0], dim=0)

        # 第一阶段残差
        residual_input = torch.cat([u_repeat, dts_all], dim=1)  # shape: (3N, input_dim)
        u_all = self.residual_block(residual_input)  # shape: (3N, 2)
        # u_all = self.deeponet(u_repeat, dts_all)
        u1, u2a, u2b = torch.chunk(u_all, 3, dim=0)

        # N0 steps
        u1 = self.N0_SCHEME.single_step(u1, dt=dts, mu=self.mu, sigma=self.sigma)
        u2a = self.N0_SCHEME.single_step(u2a, dt=2.0*dts/3.0, mu=self.mu, sigma=self.sigma)
        u2b = self.residual_block(torch.cat([u2a, 2.0*dts/3.0], dim=-1))
        # u2b = self.deeponet(u2a, 2.0*dts/3.0)
        u2 = self.N0_SCHEME.single_step(u2b, dt=dts/3.0, mu=self.mu, sigma=self.sigma)

        # 最终组合
        return -0.125 * u1 + 1.125 * u2
        # return self.linear(torch.cat([u1, u2], dim=-1)).squeeze(-1)
        # u_out = self.linear(u1, u2)
        # return u_out
        # return self.linear(torch.cat([u1, u2], dim=-1)).squeeze(-1)
    ...
    def predict(self, x):
        ...
        u = -0.125 * u1 + 1.125 * u2
        # u = self.linear(torch.cat([u1, u2], dim=-1)).squeeze(-1)
        # u = self.linear(u1, u2)
...
class ODEPairDataset(Dataset):
    def __init__(self, input_file, output_file, mu=None, sigma=None):
        raw_inputs  = np.load(input_file)[:1000, :]
        raw_outputs = np.load(output_file)[:1000, :]
